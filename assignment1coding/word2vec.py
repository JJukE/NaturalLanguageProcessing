...
def naiveSoftmaxLossAndGradient(
    ...
    diff = prob.copy()
    diff[outsideWordIdx] = diff[outsideWordIdx] - 1 # y_hat - y
...
def negSamplingLossAndGradient(
    ...
    loss -= np.log(sigmoid(np.dot(u_o, centerWordVec)))
    
    gradCenterVec = np.zeros(centerWordVec.shape)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradCenterVec += u_o * (sigmoid(np.dot(u_o, centerWordVec)) - 1)
    gradOutsideVecs[outsideWordIdx] = centerWordVec * (sigmoid(np.dot(u_o, centerWordVec)) - 1)

    # summation
    for i in range(K):
        neg_idx = indices[i + 1]
        u_w_s = outsideVectors[neg_idx]
        loss -= np.log(sigmoid(-np.dot(u_w_s, centerWordVec)))
        gradCenterVec += u_w_s * (1 - sigmoid(-np.dot(u_w_s, centerWordVec)))
        gradOutsideVecs[neg_idx] += centerWordVec * (1 - sigmoid(-np.dot(u_w_s, centerWordVec)))
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        loss_temp, g_c_temp, g_o_temp = word2vecLossAndGradient(v_c, outside_idx, outsideVectors, dataset)
        loss += loss_temp
        gradCenterVecs[center_idx] += g_c_temp
        gradOutsideVectors += g_o_temp
    ### END YOUR CODE

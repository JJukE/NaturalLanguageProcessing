...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 0. Retrieve the element of self.data
        doc = self.data[idx]
        
        # 1. Randomly truncate the document (4 ~ int(self.block_size*7/8) characters)
        ...
        masked_content_index = int(torch.randint(low=0, high=trunc_len - int(trunc_len/4) + 1, size=(1,))[0])
        ...
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + \
            self.PAD_CHAR*(self.block_size - len(prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content) + 1)

        # 4. Use masked_string to construct the input and output example pair
        x = masked_string[:-1]
        y = masked_string[1:]
        
        # 5. Encode the resulting input and output strings with defined vocabulary
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in y], dtype=torch.long)
        return x, y
